...
export default function App() {
 const [activeTab,setActiveTab]=useState<TabKey>('world'); const [moreOpen,setMoreOpen]=useState(false); const [snapshot,setSnapshot]=useState<CassidySnapshot|null>(null); const [scenarioState,setScenarioState]=useState({visible:false,scenarioKey:'ja-kyoto-tea-order'}); const [atmospherePhase,setAtmospherePhase]=useState<WorldAtmospherePhase>(currentAtmospherePhase); const screenMotion=useRef(new Animated.Value(1)).current;
 ...
 const handleTravel=async(worldId:string,placeId:string)=>{await worldPresenceEngine.travel(worldId as 'ja'|'es'|'fr'|'ko',placeId);setActiveTab('world');setMoreOpen(false);};
 const render=()=>{switch(activeTab){case'home':return <HomeScreen snapshot={snapshot} onNavigate={handleNavigate} onStartScenario={handleStartScenario}/>;case'cassidy':return <CassidyHomeScreen/>;case'study':return <StudyScreen/>;case'world':return <LivingValleyScreen onStartScenario={handleStartScenario}/>;case'journey':return <LivingJourneyScreen onTravel={handleTravel}/>;case'museum':return <MemoryMuseumScreen/>;case'characters':return <CharacterScreen/>;case'settings':return <SettingsScreen/>;default:return <LivingValleyScreen onStartScenario={handleStartScenario}/>;}};
