...
export default function App() {
  ...
  const [atmospherePhase, setAtmospherePhase] = useState<WorldAtmospherePhase>(currentAtmospherePhase);
  const screenMotion = useRef(new Animated.Value(1)).current;

  useEffect(() => { const stopWorldReactor = startLivingWorldReactor(); return stopWorldReactor; }, []);
  useEffect(() => { loadCassidySnapshot().then(setSnapshot); }, [activeTab]);
  useEffect(() => {
  ...
  useEffect(() => {
    const sync = () => setAtmospherePhase(currentAtmospherePhase());
    sync();
