"""Import a genuine Cassidy source asset into the production workspace.

The preferred source is a native Blender .blend because it preserves rig,
actions, shape keys and Blender-native relationships better than GLB. GLB/GLTF
remains supported for external intake.

This is an intake tool, not a character generator: it never creates human
...
def _argument_path() -> Path:
    ...
    if path.stat().st_size <= 0: raise ValueError("Cassidy source asset is empty")
    return path
...
def _snapshot(path: Path) -> Path:
    destination = Path(__file__).resolve().parents[2] / SOURCE_SNAPSHOT
...
def main():
    report = import_source(_argument_path())
