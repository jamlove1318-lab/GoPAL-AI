...
export class WorldEngine {
  ...
  async listLocations(worldId: string): Promise<LocationsRow[]> { const virtual = virtualLanguageLocations(worldId); const local = await LocalStore.getLocations(); const scoped = local.filter((location: LocationsRow) => !location.world_id || location.world_id === worldId); if (!isSupabaseConfigured) return mergeLocations(scoped, virtual); const { data } = await supabase.from('locations').select('*').eq('world_id', worldId); return mergeLocations(data ?? [], mergeLocations(scoped, virtual)); }
