...
export class ResidentRelationshipEngine{
 async get(residentId:string):Promise<ResidentRelationship>{const stored=await worldStateEngine.get(key(residentId));if(!stored||typeof stored!=='string')return empty(residentId);try{return JSON.parse(stored) as ResidentRelationship;}catch{return empty(residentId);}}
 async recordEncounter(residentId:string):Promise<ResidentRelationship>{const r=await this.get(residentId);r.familiarity+=1;r.tone=toneFor(r);await worldStateEngine.set(key(residentId),JSON.stringify(r));return r;}
 async recordChoice(residentId:string,choice:string):Promise<ResidentRelationship>{const r=await this.get(residentId);r.lastChoice=choice;if(choice!=='wander')r.sharedMoments+=1;if(choice==='help'){r.helped+=1;r.trust+=2;}else if(choice==='stay'||choice==='ask')r.trust+=1;r.tone=toneFor(r);await worldStateEngine.set(key(residentId),JSON.stringify(r));return r;}
 async greeting(residentId:string,name:string):Promise<string>{const r=await this.get(residentId);if(r.tone==='trusted')return `${name} lights up when they notice you. They seem genuinely glad you came back.`;if(r.tone==='warm')return `${name} recognizes you immediately and makes room for you in the moment.`;if(r.tone==='familiar')return `${name} gives you a small knowing smile. You have met here before.`;return `${name} notices you and waits to see what you will do.`;}
}
