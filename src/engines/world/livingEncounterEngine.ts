...
export class LivingEncounterEngine {
  private readonly journey = new JourneyEngine(); private readonly seen = new Set<string>();
  async approach(resident: LivingResident, userId = 'local-explorer-user'): Promise<LivingEncounterResult> {
    const eventKey = `${resident.id}:${resident.locationKey}:${resident.activity}`; const firstTime = !this.seen.has(eventKey); this.seen.add(eventKey);
  ...
  async choose(result: LivingEncounterResult, choice: EncounterChoice, userId = 'local-explorer-user'): Promise<{ title: string; detail: string; scenario?: string }> {
    if (choice.id !== 'wander') { eventBus.emit('world:residentMoment', { residentId: result.residentId, residentName: result.residentName, choice: choice.id, userId }, 'world'); await this.journey.recordEvent(userId, 'living_encounter_engine', 'world:residentMoment', { residentId: result.residentId, residentName: result.residentName, encounter: result.eventKey, choice: choice.id, label: choice.label }); }
...
function choicesFor(resident: LivingResident): EncounterChoice[] { const scenario = resident.locationKey.includes('cafe') ? 'scen-cafe-order' : resident.locationKey.includes('library') ? 'scen-library-inquiry' : resident.locationKey.includes('market') ? 'scen-market-browse' : undefined; return [{ id:'stay',label:'Stay for a while',detail:`You slow down and let ${resident.name}'s world become part of yours for a moment.`,learningScenario:scenario},{id:'ask',label:'Ask about it',detail:`${resident.name} begins explaining what they are doing, giving the moment somewhere new to go.`,learningScenario:scenario},{id:'help',label:'Offer to help',detail:'You step into the activity instead of watching from outside. What you learn now has a reason.',learningScenario:scenario},{id:'wander',label:'Keep wandering',detail:'Leave the encounter open and continue exploring.'}]; }
