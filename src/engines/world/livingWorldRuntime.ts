...
export class LivingWorldRuntime {
  ...
  async load(userId: string): Promise<WorldSnapshot> {
    ...
    await this.worldEngine.saveState(userId, {
      ...
      locationId: simulation.locationId ?? resolved.location?.id ?? undefined,
      timeOfDay: this.timeOfDayFromPhase(simulation.phase),
      season: this.seasonOf(simulation.season),
      weather: simulation.weather,
      lastActiveAt: now,
    });
    const refreshed = (await this.worldEngine.loadState(userId)) ?? { ...resolved, lastActiveAt: now };
    const basic = refreshed.location ? await this.worldEngine.getRevisitDifference(refreshed.location.id) : { note: null, visitCount: 0 };
    return { resolved: { ...refreshed, lastActiveAt: now }, simulation, continuity: this.buildContinuity(basic, previousLastActiveAt, simulation.season) };
  }
  ...
  async changeLocation(userId: string, locationId: string): Promise<WorldSnapshot> {
    ...
    const simulation = await this.simulation.advance(userId, worldId, locationId, elapsedMinutes);
    const now = new Date().toISOString();
    await this.worldEngine.saveState(userId, {
      worldId,
      locationId,
  ...
  async getRevisitDifference(locationKey: string) { return this.worldEngine.getRevisitDifference(locationKey); }
  async getRevisitStats() { return this.worldEngine.getRevisitStats(); }

  private timeOfDayFromPhase(phase: LivingWorldSnapshot['phase']): ResolvedWorldState['timeOfDay'] {
  ...
  private seasonOf(season: string): ResolvedWorldState['season'] {
  ...
  private seasonFor(date: Date): string {
