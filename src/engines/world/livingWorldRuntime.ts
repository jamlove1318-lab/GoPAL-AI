...
export interface WorldSnapshot { resolved: ResolvedWorldState; simulation: LivingWorldSnapshot; continuity: { note: string | null; visitCount: number }; }
...
export class LivingWorldRuntime {
  ...
  async load(userId: string): Promise<WorldSnapshot> {
    ...
    const continuity = resolved.location ? await this.worldEngine.getRevisitDifference(resolved.location.id, userId) : { note: null, visitCount: 0 };
  ...
  async changeLocation(userId: string, locationId: string): Promise<WorldSnapshot> {
    ...
    const continuity = await this.worldEngine.getRevisitDifference(locationId, userId);
