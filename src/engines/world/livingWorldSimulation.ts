...
const weatherFor=(seed:number):WorldWeather=>['clear','cloudy','wind','rain','snow'][Math.abs(seed)%5] as WorldWeather;
export class LivingWorldSimulation{
 ...
 async advance(userId:string,worldId:string,locationId:string|null,elapsedMinutes=0):Promise<LivingWorldSnapshot>{const previous=await this.get(userId);const now=new Date();const day=Math.floor(now.getTime()/86400000);const seed=Math.floor(now.getTime()/3600000)+day;const snapshot={worldId,locationId,phase:phaseFromHour(now.getHours()),weather:weatherFor(seed),season:this.resolveSeason(now),day,visits:(previous?.visits??0)+(locationId&&previous?.locationId!==locationId?1:0),lastUpdatedAt:now.toISOString()};try{await AsyncStorage.setItem(keyFor(userId),JSON.stringify(snapshot));}catch{}if(locationId&&previous?.locationId!==locationId)eventBus.emit('world:destinationLifeShifted',{worldId,placeId:locationId,elapsedMinutes:Math.max(0,elapsedMinutes),visits:snapshot.visits,phase:snapshot.visits<=1?'first-visit':snapshot.visits>=5?'returning':'recent'},'world');return snapshot;}
 private resolveSeason(date:Date){const month=date.getMonth();if(month===11||month<=1)return'winter';if(month<=4)return'spring';if(month<=7)return'summer';return'autumn';}
}
