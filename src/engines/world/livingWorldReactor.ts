...
export function startLivingWorldReactor(): () => void {
  ...
  const offLearning = eventBus.on('learning:sessionCompleted', (payload) => {
    ...
    void react(
      `learning:${payload.sessionId}`,
      'living-bonsai',
      `Grew after the learner completed a ${details}.`,
    );
  });

  const offDiscovery = eventBus.on('discovery:made', (payload) =>
    void react(
      `discovery:${payload.discoveryId}`,
      'living-bonsai',
      `Stirred when the learner discovered ${payload.type}: ${payload.ref}.`,
    ),
  );

  const offReturn = eventBus.on('world:returned', (payload) =>
    void react(
      `return:${payload.userId}:${payload.lastActiveAt}`,
      'living-radio',
      'Welcomed the learner home again after time away.',
    ),
  );
  ...
  const offConversation = eventBus.on('conversation:completed', (payload) =>
    void react(
      ...
      'living-radio',
      `Remembered conversation ${payload.conversationId} from the journey.`,
    ),
  );
  ...
  const offAchievement = eventBus.on('achievement:earned', (payload) =>
    void react(
      ...
      'living-bonsai',
      `Grew after the learner earned achievement ${payload.achievementId}.`,
    ),
  );
  ...
  const offQuest = eventBus.on('quest:completed', (payload) =>
    void react(
      ...
      'living-bonsai',
      `Grew after quest ${payload.questId} was completed.`,
    ),
  );

  const offStory = eventBus.on('story:progressed', (payload) =>
    void react(
      `story:${payload.storyId}:${payload.node}`,
      'living-radio',
      `Recorded a story turning point at ${payload.node}.`,
    ),
  );

  const offLocationUnlocked = eventBus.on('location:unlocked', (payload) =>
    void react(
      `location-unlocked:${payload.locationId}`,
      'living-bonsai',
      `Responded when ${payload.locationId} became part of the learner's world.`,
    ),
  );
  ...
  const offTravel = eventBus.on('world:locationChanged', (payload) => {
    ...
    void react(
      `travel:${payload.userId}:${payload.previousLocationId ?? 'start'}:${payload.locationId}`,
      'living-radio',
      `Recorded the learner's journey to ${payload.locationId}.`,
    );
  });

  const offWorldEvent = eventBus.on('world:eventStarted', (payload) =>
    void react(
      `world-event:${payload.eventId}`,
      'living-radio',
      `Noticed world event ${payload.eventId} beginning in ${payload.worldId}.`,
    ),
  );
  ...
  const offMemory = eventBus.on('memory:recorded', (payload) =>
    void react(
      ...
      'living-radio',
      `Held onto a new ${payload.layer} memory from the learner's journey.`,
    ),
  );
