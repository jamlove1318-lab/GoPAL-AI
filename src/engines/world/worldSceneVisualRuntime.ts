...
export function createWorldSceneVisualRuntime(worldId:LanguageWorldId,placeId:string,residentId?:string,date=new Date()):WorldSceneVisualRuntimeState{const scene=createWorldSceneRuntime(worldId,placeId,residentId);const simulation=createWorldSimulationSnapshot(worldId,placeId,date);const visual=buildArrivalVisualState(worldId,placeId,scene.arrival.backgroundKey,scene.arrival.atmosphere,'establishing');return{scene,simulation,visual,animation:null,interaction:{enabled:false,typingAlwaysAvailable:true,inputModes:['typed','spoken','guided'],residentTurn:'idle',lastInput:null}};}
