...
const KEY='gopal:destination-continuity:v1';
...
export class DestinationContinuityEngine{
 async enter(worldId:LanguageWorldId,placeId:string,now=new Date()):Promise<DestinationReturn>{
  const key=`${KEY}:${worldId}:${placeId}`;
