...
export function decideCassidyAction(input:CassidyAutonomyInput):CassidyAutonomyDecision{
  ...
  if(input.opportunity?.kind==='discovery')return{action:'suggest',mood:'curious' as CassidyMood,priority:65,reason:'A discovery is available, but Cassidy leaves the decision to the learner.'};
