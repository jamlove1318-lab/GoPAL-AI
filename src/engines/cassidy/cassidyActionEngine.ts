...
export type CassidyAction={action:CassidyAutonomyDecision['action'];mood:CassidyAutonomyDecision['mood'];text:string;canInterrupt:boolean;priority:number;lifeActivity?:CassidyLifeActivity;invitation?:boolean;visualAction:'idle'|'talking'|'waving'|'walking'};
const LIFE_TEXT:Record<CassidyLifeActivity,string>={wandering:'Cassidy is wandering through the world, following her curiosity.',cafe:'Cassidy found a quiet café and decided to stay for a while.',reading:'Cassidy is sitting somewhere peaceful, absorbed in a book.', 'watching-rain':'Cassidy stopped to watch the rain for a little while.',stargazing:'Cassidy is looking up at the sky, taking in the stars.',dreaming:'Cassidy is lost in one of her strange little dreams.',storytelling:'Cassidy has a story she wants to share with you.',adventure:'Cassidy discovered something interesting and wants to explore it.',helping:'Cassidy noticed you could use a little help.',resting:'Cassidy is simply taking a quiet moment for herself.',discovering:'Cassidy noticed something she had never seen before.',celebrating:'Cassidy is enjoying what you accomplished together.'};
const LIFE_VISUAL:Record<CassidyLifeActivity,CassidyAction['visualAction']>={wandering:'walking',cafe:'idle',reading:'idle','watching-rain':'idle',stargazing:'idle',dreaming:'idle',storytelling:'talking',adventure:'walking',helping:'talking',resting:'idle',discovering:'idle',celebrating:'waving'};
export function executeCassidyDecision(decision:CassidyAutonomyDecision):CassidyAction{
 const lifeActivity=decision.lifeActivity; const action=decision.action;
 ...
 if(lifeActivity)void cassidyLifeStateEngine.begin({activity:lifeActivity,mood:result.mood,worldId:decision.worldId??'emerald-valley',destinationId:decision.destinationId,invitation:result.invitation});
 eventBus.emit('cassidy:autonomyActed',{...result,worldId:decision.worldId,destinationId:decision.destinationId},'cassidy');
 return result;
}
