...
export function WorldDestinationExploration({worldId,placeId,onStartScenario,onClose}:{worldId:any;placeId:string;onStartScenario?:(scenarioKey:string)=>void;onClose?:()=>void}){
 ...
 const choose=async(h:WorldPlaceHotspot)=>{if(busy||h.kind==='locked')return;setBusy(true);try{const result=await worldHotspotProgressionEngine.resolve(h);setVisible(await worldHotspotRevealEngine.getVisible(placeId).then(s=>s.visible));setSelected({...h,...(result.experience?{experience:result.experience}: {})} as WorldPlaceHotspot);}finally{setBusy(false);}};
 const enter=async()=>{if(!selected)return;const scenarioIds=(selected as WorldPlaceHotspot&{scenarioIds?:string[]}).scenarioIds??[];await worldHotspotProgressionEngine.complete(selected.id);await refresh();setSelected(null);if(scenarioIds[0])onStartScenario?.(scenarioIds[0]);};
