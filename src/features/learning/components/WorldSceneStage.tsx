...
export function WorldSceneStage({worldId,placeId,residentId,showResident}:{worldId:any;placeId:string;residentId?:string;showResident:boolean}){
 ...
 return <View className="absolute inset-0 overflow-hidden bg-slate-950"><PlaceEnvironmentView placeId={placeId}/><View className="absolute inset-0 bg-emerald-950/25"/><View className="absolute -left-16 -top-12 h-64 w-64 rounded-full bg-emerald-500/10"/><View className="absolute -bottom-20 -right-10 h-72 w-72 rounded-full bg-cyan-500/10"/>
