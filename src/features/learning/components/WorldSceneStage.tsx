...
export function WorldSceneStage({worldId,placeId,residentId,showResident}:{worldId:any;placeId:string;residentId?:string;showResident:boolean}){
 ...
 useEffect(()=>{const sequence=showResident?Animated.sequence([Animated.timing(camera,{toValue:0.45,duration:900,useNativeDriver:true}),Animated.timing(camera,{toValue:1,duration:1200,useNativeDriver:true})]):Animated.timing(camera,{toValue:0,duration:600,useNativeDriver:true});sequence.start();return()=>sequence.stop();},[showResident,camera]);
 ...
 return <View className="absolute inset-0 overflow-hidden bg-slate-950"><Animated.View style={{flex:1,transform:[{scale:cameraScale},{translateX:cameraX}]}}><PlaceEnvironmentView placeId={placeId} weather={weather}/></Animated.View><View className="absolute inset-0 bg-emerald-950/25"/><View className="absolute -left-16 -top-12 h-64 w-64 rounded-full bg-emerald-500/10"/><View className="absolute -bottom-20 -right-10 h-72 w-72 rounded-full bg-cyan-500/10"/>
