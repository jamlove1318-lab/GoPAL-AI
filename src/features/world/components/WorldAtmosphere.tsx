...
interface WorldAtmosphereProps { phase: WorldAtmospherePhase; weather?: WorldWeatherMood; location?: WorldLocationMood; intensity?: number; season?: Season; }
...
/** Quiet visual atmosphere: canonical environment + place create depth without another UI panel. */
export function WorldAtmosphere({ phase, weather, location = 'unknown', intensity = 1, season }: WorldAtmosphereProps) {
  const drift = useRef(new Animated.Value(0)).current;
  const breathe = useRef(new Animated.Value(0)).current;
  const environment = new EnvironmentEngine().resolve();
  const resolvedSeason = season ?? environment.season;
  const resolvedWeather: WorldWeatherMood = weather ?? (environment.weather === 'snow' ? 'snow' : environment.weather === 'rain' || environment.weather === 'light-rain' ? 'rain' : environment.weather === 'breeze' || environment.weather === 'windy' ? 'cloudy' : 'clear');
  ...
  const base = Math.min(0.34, Math.max(0, (phaseOpacity[phase] + locationOpacity[location] + seasonOpacity[resolvedSeason]) * intensity));
