...
interface WorldAtmosphereProps {
  phase: WorldAtmospherePhase;
  weather?: WorldWeatherMood;
  intensity?: number;
}

const phaseOpacity: Record<WorldAtmospherePhase, number> = { dawn: 0.12, morning: 0.05, afternoon: 0, dusk: 0.13, night: 0.22 };

/** Quiet visual atmosphere: depth and motion, never another UI panel. */
export function WorldAtmosphere({ phase, weather = 'clear', intensity = 1 }: WorldAtmosphereProps) {
  ...
  const base = Math.min(0.28, Math.max(0, phaseOpacity[phase] * intensity));
  ...
  return (
    <View pointerEvents="none" className="absolute inset-0 overflow-hidden">
      ...
      {weatherOpacity > 0 && <Animated.View style={{ opacity: weatherOpacity * intensity, transform: [{ translateX: Animated.multiply(translateX, 0.6) }] }} className="absolute -left-16 top-24 h-48 w-[130%] rounded-full bg-slate-200/10" />}
    </View>
  );
}
