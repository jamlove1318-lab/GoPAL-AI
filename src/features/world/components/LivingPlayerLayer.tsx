 ...
 * there is intentionally no interaction card, nameplate, or instruction HUD.
 */
export function LivingPlayerLayer({locationId='emerald-village',runtime}:{buildings?:unknown[];locationId?:string;runtime?:LivingWorldRuntime}){
 ...
 const worldRuntime=runtime??useMemo(()=>new LivingWorldRuntime(locationId),[locationId]);
 ...
 useEffect(()=>{
  const point=worldRuntime.getPlayer();
  target.x=point.x;target.y=point.y;
  position.setValue(point);setDepth(point.y);
 },[worldRuntime,position,target]);
 ...
 const stop=()=>{
  movement.active=false;movement.x=0;movement.y=0;
  Animated.spring(joystick,{toValue:{x:0,y:0},useNativeDriver:true,tension:140,friction:10}).start();
  if(tick.current)clearInterval(tick.current);
  if(depthTick.current)clearInterval(depthTick.current);
  tick.current=undefined;depthTick.current=undefined;
  setDepth(target.y);
  worldRuntime.setPlayerPosition(target.x,target.y);
 };
 ...
 const start=()=>{
  ...
  tick.current=setInterval(()=>{
   const next=worldRuntime.movePlayer(movement.x*.72,movement.y*.72);
   target.x=next.x;target.y=next.y;position.setValue(next);
  },32);
