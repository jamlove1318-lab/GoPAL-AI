...
export function LivingPlayerLayer({
  ...
  const ownedRuntime = useMemo(() => new LivingWorldRuntime(locationId), [locationId]);
  ...
  useEffect(() => {
    ...
    setDepth(point.y);
    setNearbyInteraction(worldRuntime.getNearbyInteraction());
  ...
  const updateNearby = (point: Point) => {
    worldRuntime.setPlayerPosition(point.x, point.y);
    setNearbyInteraction(worldRuntime.getNearbyInteraction());
  };
  ...
  const start = () => {
    ...
    tick.current = setInterval(() => {
      const next = worldRuntime.movePlayer(movement.x * 0.72, movement.y * 0.72);
      target.x = next.x;
      target.y = next.y;
  ...
  const activateNearby = () => {
    ...
    if (selectedAction) worldRuntime.interact(selectedAction);
    setNearbyInteraction(worldRuntime.getNearbyInteraction());
...
function SvgPlayer() { return <Svg width={68} height={80} viewBox="0 0 68 80"><Ellipse cx="34" cy="74" rx="24" ry="5" fill="#08120f" opacity={0.42}/><Path d="M22 62L27 74M46 62L41 74" stroke="#202a30" strokeWidth="7" strokeLinecap="round"/><Path d="M16 63Q17 39 34 36Q51 39 52 63Z" fill="#294e5a"/><Path d="M20 60L34 42L48 60" fill="#5b93a0" opacity={0.55}/><Path d="M19 45Q34 35 49 45" stroke="#d7b079" strokeWidth="4" opacity={0.8}/><Circle cx="34" cy="27" r="15" fill="#d8a57f"/><Path d="M20 28Q19 10 34 9Q50 10 49 28Q42 20 34 20Q26 20 20 28Z" fill="#1d2024"/><Path d="M22 20Q28 7 39 10Q47 12 49 22Q39 17 22 20Z" fill="#34383d"/><Circle cx="29" cy="28" r="1.7" fill="#20262b"/><Circle cx="39" cy="28" r="1.7" fill="#20262b"/><Path d="M30 35Q34 38 38 35" stroke="#9b5d58" strokeWidth="1.5" strokeLinecap="round" fill="none"/><Path d="M48 44Q58 47 59 57" stroke="#d6a86a" strokeWidth="4" strokeLinecap="round"/><Rect x="29" y="40" width="10" height="3" rx="1.5" fill="#d6a86a" opacity={0.7}/></Svg>; }
