...
const BUILDINGS: GameWorldBuilding[] = [
  { id: 'sanctuary', x: 14, y: 70, scale: .95, label: 'Sanctuary' },
  { id: 'cafe', x: 47, y: 41, scale: 1.05, label: 'Komorebi Café' },
  { id: 'library', x: 75, y: 23, scale: .92, label: 'Whispering Library' },
  { id: 'market', x: 78, y: 68, label: 'Lantern Market' },
  { id: 'garden', x: 31, y: 82, scale: .9, label: 'Whisper Garden' },
];
...
function PhysicalBuilding({ building, night }: { building: GameWorldBuilding; night: boolean }) {
  ...
  return <Pressable disabled={!building.onPress} onPress={building.onPress} style={{position:'absolute',left:`${building.x}%`,top:`${building.y}%`,width:w,height:w,marginLeft:-w/2,marginTop:-w*.75,zIndex:Math.round(building.y*10)}}>
    ...
    {building.label && <Text style={styles.label}>{building.label}</Text>}
  </Pressable>;
}
...
const styles=StyleSheet.create({root:{...StyleSheet.absoluteFillObject,overflow:'hidden'},ground:{...StyleSheet.absoluteFillObject},label:{position:'absolute',bottom:-2,alignSelf:'center',fontSize:9,fontWeight:'700',color:'rgba(255,255,255,.78)',textShadowColor:'rgba(0,0,0,.7)',textShadowOffset:{width:0,height:1},textShadowRadius:3}});
