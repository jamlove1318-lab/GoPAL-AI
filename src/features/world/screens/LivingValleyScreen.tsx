...
const PLACES: Place[] = [
  ...
  { name: 'Komorebi Café', icon: '☕', x: '47%', y: '38%', scene: 'Warm windows glow. Someone is talking inside.', scenario: 'scen-cafe-order' },
  { name: 'Whispering Library', icon: '📚', x: '73%', y: '20%', scene: 'A few pages turn somewhere behind the windows.', scenario: 'scen-library-inquiry' },
  { name: 'Lantern Market', icon: '🏮', x: '78%', y: '67%', scene: 'Stalls are gathering light as the valley changes.', scenario: 'scen-market-browse' },
  { name: 'Whisper Garden', icon: '🌿', x: '31%', y: '79%', scene: 'The bamboo moves even when you stand still.' },
];
  ...
  return <SafeAreaView className="flex-1 bg-transparent"><View className="flex-1 overflow-hidden">
    <LivingWorldViewport>
      <View className="flex-1 overflow-hidden">
        <Animated.View pointerEvents="none" style={{ transform: [{ translateX: cloud.interpolate({ inputRange: [0, 1], outputRange: [-70, 110] }) }] }} className="absolute left-[-10%] top-8 h-24 w-72 rounded-full bg-white/5" />
        <Animated.View pointerEvents="none" style={{ opacity: cloud.interpolate({ inputRange: [0, 1], outputRange: [0.08, 0.22] }) }} className="absolute right-[-20%] top-32 h-16 w-60 rounded-full bg-indigo-300/20" />
    ...
    <View pointerEvents="none" className="absolute left-5 right-5 top-4 z-20"><View className="flex-row items-center"><Compass size={14} color="#6ee7b7" /><Text className="ml-2 text-[10px] font-bold uppercase tracking-[2.5px] text-emerald-300">Emerald Valley</Text></View><Text className="mt-2 text-[31px] font-black text-white">Where will you wander?</Text><Text className="mt-1 max-w-[320px] text-sm leading-5 text-slate-400">Drag the world. Pinch to look closer. Someone is always doing something.</Text></View>
    <View pointerEvents="none" className="absolute right-5 top-6 z-30 h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-slate-950/40"><Maximize size={15} color="#94a3b8" /></View>
    <View pointerEvents="none" className="absolute bottom-5 left-5 right-5 z-40 flex-row items-center justify-between"><View><Text className="text-[10px] uppercase tracking-[1.8px] text-slate-500">Tonight in the valley</Text><Text className="mt-1 text-sm text-slate-200">Things can change while you are away.</Text></View><View className="h-11 w-11 items-center justify-center rounded-full border border-emerald-300/25 bg-emerald-400/10"><Sparkles size={18} color="#6ee7b7" /></View></View>
