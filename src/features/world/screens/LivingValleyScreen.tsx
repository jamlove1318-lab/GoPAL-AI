...
import{LivingWorldConsequenceLayer}from'../components/LivingWorldConsequenceLayer';
import{LivingWorldHotspotLayer}from'../components/LivingWorldHotspotLayer';
...
/**
 * Generic living-world surface. The selected location is canonical data; the
 * renderer contains no world-specific building list or presentation shell.
 */
export function LivingValleyScreen({locationId='emerald-village'}:Props){
 ...
 useEffect(()=>{
  const unsubscribe=runtime.events.subscribe(()=>setRevision(value=>value+1));
  const tick=()=>{const now=new Date();setWorldTime(now);runtime.tick(now.getTime());};
  tick();
