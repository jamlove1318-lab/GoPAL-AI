...
export const CASSIDY_2D_REQUIRED_LAYERS = [
  'head',
  'ears',
  'neck',
  'eyes',
...
export const CASSIDY_2D_REQUIRED_EXPRESSIONS = [
  'neutral',
  'happy',
  'curious',
  'excited',
...
export const CASSIDY_2D_REQUIRED_ANIMATIONS = [
  'idle-breath',
  'walk',
  'run',
  'talk',
...
export const CASSIDY_2D_REQUIRED_CHARM_STATES = [
  'normal',
  'curious',
  'learning',
  'discovery',
...
const includesAll = <T extends string>(required: readonly T[], actual: readonly T[]) =>
...
export function validateCassidy2DProductionPack(pack: Cassidy2DProductionPack): boolean {
  return (
    pack.id === 'cassidy-canonical-2d' &&
    pack.contractVersion === CASSIDY_2D_CONTRACT_VERSION &&
    pack.identityVersion === CASSIDY_2D_IDENTITY_VERSION &&
    pack.sourceReference === 'cassidy-character-reference.md' &&
    pack.format === 'layered-2d-puppet' &&
    pack.masterResolution.width >= 2000 &&
    pack.masterResolution.height >= 3000 &&
    includesAll(CASSIDY_2D_REQUIRED_LAYERS, pack.layers) &&
    includesAll(CASSIDY_2D_REQUIRED_EXPRESSIONS, pack.expressions) &&
    includesAll(CASSIDY_2D_REQUIRED_ANIMATIONS, pack.animations) &&
    includesAll(CASSIDY_2D_REQUIRED_CHARM_STATES, pack.charmStates) &&
    pack.runtimeAssetVersion.trim().length > 0 &&
    pack.humanApproved
  );
}
...
export function registerCassidy2DProductionPack(pack: Cassidy2DProductionPack): void {
  if (!validateCassidy2DProductionPack(pack)) {
    throw new Error('Cassidy 2D production pack rejected: canonical requirements are incomplete.');
  }
  registeredPack = pack;
}

export function getCassidy2DProductionPack(): Cassidy2DProductionPack | null {
