...
export function resolveCinematicTarget(objects: WorldObjectDefinition[], focus: 'landmark' | 'resident' | 'transport' | 'environment' | 'departure' | 'establishing'): WorldCinematicTarget {
  ...
  const landmark = find(object => object.category === 'building' && (object.tags?.includes('landmark') || object.tags?.includes('signature') || Boolean(object.metadata?.label))) ?? find(object => object.category === 'building');
  const resident = find(object => object.category === 'character');
  const transport = find(object => object.category === 'transport' || object.category === 'vehicle');
  const environment = find(object => object.category === 'nature' || object.tags?.includes('environment'));
  const target = focus === 'landmark' ? landmark : focus === 'resident' ? resident : focus === 'transport' ? transport : focus === 'environment' ? environment : focus === 'departure' ? transport ?? landmark : landmark ?? environment ?? resident;
  if (!target) return { x: 50, y: 50, scale: 1, reason: 'location-center-fallback' };
  return { x: target.transform.x, y: target.transform.y, scale: Math.max(.9, Math.min(1.35, target.transform.scale ?? 1)), rotation: target.transform.rotation, objectId: target.id, reason: `${focus}:${target.id}` };
}