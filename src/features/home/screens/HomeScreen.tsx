  ...
  return (
    <SafeAreaView className="flex-1 bg-transparent">
      <ScrollView className="flex-1 px-5 pt-3" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
        <View className="flex-row items-center justify-between"><View><Text className="text-xs font-semibold uppercase tracking-wider text-emerald-400">{state?.world?.display_name ?? 'GoPAL Living World'}</Text><Text className="mt-0.5 text-2xl font-bold text-white">Your Sanctuary</Text></View><Pressable onPress={() => setShowFestivalModal(true)} className="flex-row items-center gap-1.5 rounded-full bg-slate-800/80 px-3 py-1.5 border border-slate-700 active:bg-slate-700">{timeIcon}<Text className="text-xs font-medium capitalize text-slate-200">{season} · {timeOfDay} ✨</Text></Pressable></View>
        ...
        {continuity && (continuity.newDay || continuity.isNewSeason || continuity.recap.length > 0) && <View className="mt-4 rounded-2xl border border-rose-500/40 bg-rose-950/25 p-4"><Text className="text-sm font-bold text-white">{continuity.newDay ? 'Your world entered a new day.' : 'The world kept living while you were gone.'}</Text>{continuity.recap.map((line, i) => <Text key={i} className="mt-1 text-xs text-slate-300">• {line}</Text>)}<Text className="mt-2.5 text-[11px] italic text-slate-400">Nothing required your attention — the world simply continued.</Text></View>}
        {continuityCard && <View className="mt-4 rounded-2xl border border-blue-500/40 bg-blue-950/30 p-4"><Text className="text-xs font-bold uppercase tracking-wider text-blue-300">Pick Up Where You Left Off</Text><Text className="mt-2 text-sm font-bold text-white">{continuityCard.title}</Text><Text className="mt-0.5 text-xs text-slate-300">{continuityCard.promptSnippet}</Text><Pressable onPress={() => continuityCard.scenarioKey && onStartScenario ? onStartScenario(continuityCard.scenarioKey) : onNavigate?.('study')} className="mt-3 flex-row items-center justify-center gap-1.5 rounded-xl bg-blue-600 py-2.5"><Play size={13} color="#fff" /><Text className="text-xs font-bold text-white">Resume Session</Text></Pressable></View>}
        {todayMoment && <View className="mt-4 rounded-2xl border border-emerald-500/40 bg-emerald-950/40 p-4"><Text className="text-xs font-bold uppercase tracking-wider text-emerald-400">Today's Curated Moment</Text><Text className="mt-2 text-base font-bold text-white">{todayMoment.headline}</Text><Text className="mt-1 text-xs text-slate-300">{todayMoment.subtext}</Text><Pressable onPress={() => todayMoment.scenarioKey && onStartScenario ? onStartScenario(todayMoment.scenarioKey) : onNavigate?.('map', { targetLocation: todayMoment.targetLocationKey })} className="mt-3.5 flex-row items-center justify-center gap-2 rounded-xl bg-emerald-500 py-2.5"><Play size={14} color="#064e3b" /><Text className="text-xs font-bold text-emerald-950">{todayMoment.primaryActionLabel}</Text></Pressable></View>}
        <View className="mt-4 flex-row flex-wrap gap-2.5"><Pressable onPress={() => setShowKnowledgeModal(true)} className="flex-1 rounded-2xl border border-slate-800 bg-slate-900 py-3"><Text className="text-center text-xs font-semibold text-slate-200">Constellation</Text></Pressable><Pressable onPress={() => setShowCreativeModal(true)} className="flex-1 rounded-2xl border border-slate-800 bg-slate-900 py-3"><Text className="text-center text-xs font-semibold text-slate-200">Creative Studio</Text></Pressable></View>
        <View className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-4"><Text className="text-xs font-bold uppercase tracking-wider text-emerald-300">Living world</Text><Text className="mt-1 text-xs text-slate-300">Your world currently contains {livingObjects.length} living objects.</Text><Text className="mt-1 text-[10px] text-slate-500">{selfModel ?? 'The world is learning your rhythm.'}</Text></View>
        {djResult && <Text className="mt-3 text-xs text-emerald-200">{djResult}</Text>}
      </ScrollView>
      <CreativeStudioModal visible={showCreativeModal} onClose={() => setShowCreativeModal(false)} />
      <KnowledgeGraphModal visible={showKnowledgeModal} onClose={() => setShowKnowledgeModal(false)} />
      <TimeCapsuleModal visible={showTimeCapsuleModal} onClose={() => setShowTimeCapsuleModal(false)} />
      <QuestShopModal visible={showQuestShopModal} onClose={() => setShowQuestShopModal(false)} />
      <SeasonalFestivalModal visible={showFestivalModal} onClose={() => setShowFestivalModal(false)} />
      <DiscoveriesModal visible={showDiscoveriesModal} onClose={() => setShowDiscoveriesModal(false)} />
    </SafeAreaView>
  );
}
