...
import type { CassidyRuntimeProductionPackage } from './cassidyProductionPackage';
import { validateCassidyRuntimeProductionPackage } from './cassidyProductionPackage';
...
export const CASSIDY_PRODUCTION_ASSET_MANIFEST: CassidyProductionAssetManifest = {
  ...
  identityLocked: true,
  assets: [
    { id: 'cassidy-canonical-concept-v1', version: 'v1', status: 'reference-approved', canonicalReference: CASSIDY_CANONICAL_REFERENCE_URI, sourceUri: CASSIDY_CANONICAL_REFERENCE_URI, notes: 'Approved Phase 1 visual identity reference.' },
    ...
    { id: 'cassidy-model-v1', version: 'v1', status: 'pending-production', canonicalReference: 'cassidy-turnaround-v1', notes: 'Real authored model; integrate only after production validation.' },
    { id: 'cassidy-rig-v1', version: 'v1', status: 'pending-production', canonicalReference: 'cassidy-model-v1', notes: 'Full-body plus facial/eye/hand controls, embedded in the runtime model package.' },
    { id: 'cassidy-animation-v1', version: 'v1', status: 'pending-production', canonicalReference: 'cassidy-model-v1', notes: 'Canonical locomotion and emotional animation library, embedded in the runtime model package.' },
  ],
};
...
export function validateCassidyAssetManifest(manifest: CassidyProductionAssetManifest = CASSIDY_PRODUCTION_ASSET_MANIFEST): string[] {
  ...
  if (!manifest.assets.some(asset => asset.id === 'cassidy-canonical-concept-v1' && asset.status === 'reference-approved')) errors.push('Approved Cassidy canonical concept reference is missing.');
...
/** Validate and, only when valid, project a production package into the runtime registry. */
export function validateCassidyRuntimePackage(pkg: CassidyRuntimeProductionPackage): string[] {
  return validateCassidyRuntimeProductionPackage(pkg, CASSIDY_PRODUCTION_ASSET_MANIFEST).errors;
}

export function createCassidyRuntimeIntegrationPatch(pkg: CassidyRuntimeProductionPackage) {
  const errors = validateCassidyRuntimePackage(pkg);
  if (errors.length > 0) return { integrated: false as const, errors };
  return {
    integrated: true as const,
    assetSet: {
      model3dUri: pkg.modelUri,
      previewUri: CASSIDY_CANONICAL_REFERENCE_URI,
      thumbnailUri: CASSIDY_CANONICAL_REFERENCE_URI,
      rigVersion: pkg.rigVersion,
      textureVersion: pkg.textureVersion,
      animationVersion: pkg.animationVersion,
      validatedAngles: CASSIDY_PRODUCTION_SPEC.canonicalReferenceViews,
      validatedExpressions: [...CASSIDY_PRODUCTION_SPEC.expressions],
      validatedAnimations: [...CASSIDY_PRODUCTION_SPEC.animations],
    } satisfies CassidyCharacterAssetSet,
  };
}
